...
class D3MDataset:
	...
	def get_learning_data(self, view=None, problem=None):
		...
		if view.upper() == 'TRAIN' or view.upper() == 'TEST':
			...
			df = df.loc[splitsdf.index] # in multi-index case, this introduces duplicate rows (e.g., LL1_penn_fudan)
			df = df.drop_duplicates() # in multi-index case, the duplicate rows have to be deleted
			return df
	...
	def _get_learning_data_path(self):
		...
		for res in self.dsDoc['dataResources']:
			...
			if (resType=='table' or resType=='raw') and (dirname=='tables' or dirname=='raw'):
