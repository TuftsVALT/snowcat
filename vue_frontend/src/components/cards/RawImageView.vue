  ...
  <div id="raw-image-view">
    ...
    <v-container fluid grid-list-sm>
      <v-layout row wrap style="min-height: 580px;">
        <v-flex
          v-for="(bin, bindex) in imagesBinned"
          :key="bindex + '_0'"
          class="image-bin"
          v-bind:style="{width: (100 / imagesBinned.length) + '%'}"
        >
        ...
        <v-flex v-for="(bin, bindex) in imagesBinned" :key="bindex + '_1'">
          <v-flex
            xs1
            v-for="(image, index) in bin"
            :key="image.d3mIndex"
            class="image-card"
            v-bind:style="{top: (index % 6  * 12.5 + 1) +'%', left: (bindex / imagesBinned.length * 98 + Math.floor(index / 6) + 1) + '%'}"
          >
            <!-- <v-tooltip top> -->
      ...
      <v-layout row wrap>
        ...
        <v-flex xs12 sm4 class="text-xs-center">
          <v-btn v-on:click="handleLoading" :disable="imagesAvailable.length == 0" color="primary">
            Load More Images
        ...
        <v-flex xs12 sm4 class="text-xs-center text-sm-right">
          <div style="margin-top: 20px; margin-right: 10px;">
            ...
            >{{ imagesLoaded.length }} / {{ imagesAvailable.length + imagesLoaded.length}}</h5>
          </div>
        </v-flex>
      </v-layout>
    </v-container>
...
export default {
  ...
  computed: {
    ...
    dataRoot() {
      ...
      return this.$store.state.socket.dataRoot;
    },
    valName() {
  ...
  methods: {
    ...
    processImageRow(r) {
      ...
      let valInfo = _.find(
        this.data.tableInfo.columns,
        c => _.indexOf(c.role, "suggestedTarget") >= 0
      );
    ...
    loadData() {
      ...
      this.data = {};
      this.data.tableInfo = _.find(
        this.dataResources,
        res => res.resType == "table"
      );
      this.data.imageInfo = _.find(
        this.dataResources,
        res => res.resType == "image"
      );
      this.data.valInfo = _.find(
        this.data.tableInfo.columns,
        c => _.indexOf(c.role, "suggestedTarget") >= 0
      );
      if (this.isNumType) {
