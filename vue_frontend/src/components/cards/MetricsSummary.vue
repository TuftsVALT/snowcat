<template>
  <div id="data-definition-card">
    <v-data-table v-bind:headers="modelMetricHeaders" :items="modelMetrics" class="elevation-0">
      <template slot="items" slot-scope="props">
        <tr :style="getStyle(props.item.modelId)">
          <td
            class="metric-value-text text-xs-left"
            :title="props.item.modelName"
          >{{ shorten(props.item.modelName) }}</td>
    ...
    modelMetricHeaders() {
      if (!this.$store.state.socket.rawProblemDesc.about) {
        ...
        return [
          ...
          {
            text: this.$store.state.socket.rawProblemDesc.inputs
              .performanceMetrics[0].metric,
            value: "score",
            align: "center",
            sortable: true
          }
