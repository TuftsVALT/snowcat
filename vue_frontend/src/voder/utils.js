...
(function () {
    ...
    window._voder_utils.cloneObj = function(obj) {
        ...
        if (obj instanceof Array) {
            ...
            for (var i = 0, len = obj.length; i < len; i++) {
                copy[i] = window._voder_utils.cloneObj(obj[i]);
            }
            return copy;
        }
        ...
        if (obj instanceof Object) {
            var copy = {};
            for (var attr in obj) {
                if (obj.hasOwnProperty(attr)) copy[attr] = window._voder_utils.cloneObj(obj[attr]);
            }
            return copy;
        }
    ...
    window._voder_utils.arraysEqual = function(arr1, arr2) {
    ...
    window._voder_utils.getNewSpecificationVisObject = function(){
        let visObject = {};
        visObject['mark'] = "";
        visObject['x'] = {
            "attribute" : "",
    ...
    window._voder_utils.visObjectMatchesVisSpecObj = function(visObject,visSpecObject){
    ...
    window._voder_utils.visObjectsAreEquivalent = function (visObject1, visObject2) {
    ...
    window._voder_utils.sortObj = function(list, key,order) {
    ...
    window._voder_utils.getAttributesFromVisObject = function(visObj){
        let attributes = [];
    ...
    window._voder_utils.getItemsFromDataFact = function(dataFactObject){
        let items = [];
    ...
    window._voder_utils.getCategoriesFromDataFact = function(dataFactObject){
        let categories = [];
    ...
    window._voder_utils.searchMatchingDataFacts = function(searchString){
        // let itemAttributeIgnoreString = "name";
        // let itemAttributeIgnoreString = "model";
        let itemAttributeIgnoreString = window._voder_globalVars.itemAttribute.toLowerCase();
        ...
        for(var attributeCombination in window._voder_globalVars.mainSessionMap){
            for(var dataFact of window._voder_globalVars.mainSessionMap[attributeCombination]['dfObjects']){
                        ...
                        if(target instanceof Array){
                            for(var targetVal of target){
                                if(isNaN(targetVal)==true){
                                    ...
                                    //        if(dataFact.defaultHtml.toLowerCase().indexOf(itemAttributeIgnoreString)==-1 && window._voder_globalVars.displayedDataFactIds.indexOf(dataFact.id)==-1){
    ...
    window._voder_utils.isVisObjectInList = function(newVisObject,visObjectList){
        for(var visObj of visObjectList){
            if(window._voder_utils.visObjectsAreEquivalent(visObj,newVisObject)!=-1 || window._voder_utils.visObjsAreVariations(visObj, newVisObject)==true){
                return 1;
            }
        }
        return -1;
    ...
    window._voder_utils.updateVisMatchScore = function(sourceVisObj,targetVisObj){
    ...
    window._voder_utils.getUniqueAlternativeVisualizationsForFact = function(dataFact, activeVisObject){
        //console.log("==========")
        for(var visObj of dataFact.relatedVisObjects){
            window._voder_utils.updateVisMatchScore(activeVisObject,visObj);
        }
        window._voder_utils.sortObj(dataFact.relatedVisObjects,'activeVisMatchScore','d');
        let uniqueAlternativeVisObjs = [];
        for(var visObj of dataFact.relatedVisObjects){
            if(window._voder_utils.visObjectsAreEquivalent(visObj,activeVisObject)==-1 && window._voder_utils.visObjsAreVariations(visObj, activeVisObject)==false){
                //console.log(visObj.type, window._voder_utils.isVisObjectInList(visObj,uniqueAlternativeVisObjs));
                if(window._voder_utils.isVisObjectInList(visObj,uniqueAlternativeVisObjs)==-1){
    ...
    window._voder_utils.visObjsAreVariations = function(visObj1, visObj2){
