...
def generateMainDataFile(dataList, metadataMap):
    ...
    attributeCombinations = getAttributeCombinations(list(metadataMap.keys())) # get all attribute combinations to use them as keys in mainDataMap
    ...
    outputJSONFile = outputDir + 'output-mainDataMap.json'
    with open(outputJSONFile, 'w', encoding="utf8") as outfile:
        json.dump(mainDataMap, outfile)
        # outfile.write(";")
        # outfile.write("\n\n\n")
        # outfile.write("var metadataMap = ")
    ...
    outputJSONFile = outputDir + 'output-metadataMap.json'
    with open(outputJSONFile, 'w', encoding="utf8") as outfile:
        json.dump(metadataMap, outfile, indent=1)
...
if __name__ == '__main__':
    ...
    dataList = []
    dataFileReader = csv.reader(open(dataFile,'r'))
    attributes = next(dataFileReader)
