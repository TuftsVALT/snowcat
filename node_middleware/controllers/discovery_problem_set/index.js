...
var tablularTaskMappings = {
  "integer": ['classification', 'regression', 'clustering', 'collaborativeFiltering'],
  "real": ['regression', 'collaborativeFiltering'], /*['classification', 'regression', 'clustering', 'collaborativeFiltering'],*/
  "categorical": ['classification', 'regression', 'clustering', 'collaborativeFiltering'],
  "dateTime": ['classification', 'regression', 'clustering', 'collaborativeFiltering'],
  ...
  "realVector": ['classification', 'regression', 'clustering', 'collaborativeFiltering'],
  "json": ['classification', 'regression', 'clustering', 'collaborativeFiltering'],
  "geojson": ['classification', 'regression', 'clustering', 'collaborativeFiltering'],


}

var graphTaskMappings = {
  "integer": ['classification', 'regression', 'clustering', 'linkPrediction', 'vertexNomination', 'communityDetection', 'graphClustering','collaborativeFiltering'],
   //graphMatching
  ...
  "real": ['regression', 'linkPrediction', 'vertexNomination', 'communityDetection', 'graphClustering','collaborativeFiltering'],

  "categorical": ['classification', 'regression', 'clustering', 'linkPrediction', 'vertexNomination', 'communityDetection', 'graphClustering','collaborativeFiltering'],

  "string": ['classification', 'regression', 'clustering', 'linkPrediction', 'vertexNomination', 'communityDetection', 'graphClustering','collaborativeFiltering'],
  "boolean": ['classification', 'regression', 'clustering', 'linkPrediction', 'vertexNomination', 'communityDetection', 'graphClustering','collaborativeFiltering']
    ...
    if(drs["resType"] == "table"){
        drs["columns"].forEach(drc => {
            if(drc["colName"] != "d3mIndex"){
              ...
              if(drc["refersTo"]){
                dataDescMetadata.columnType.resObject.push(drc["refersTo"]["resObject"]);
              }
              else{
...
*/
  ...
  if ( _.includes(dataDescMetadata.resourceType.type, 'graph')){
      ...
      for(var i = 0; i < dataDescMetadata.columnType.id.length; i++){
            ...
            if(dataDescMetadata.columnType.type[i] == 'integer' || dataDescMetadata.columnType.type[i] == 'real' || dataDescMetadata.columnType.type[i] == 'categorical' || dataDescMetadata.columnType.type[i] == 'string' || dataDescMetadata.columnType.type[i] == 'boolean'){
                    ...
                    for (var k = 0; k < graphTaskMappings.integer.length; k++){
                      if(graphTaskMappings.integer[k] == 'classification'){
                          for(var l = 0; l < metricMappings.classification.length; l++){
                            ...
                            singleProblem.description = 'This is a graph problem with classification task';
                        ...
                        for(var l = 0; l < metricMappings.regression.length; l++){
                          ...
                          singleProblem.creationType = 'auto';
                          singleProblem.meaningful = 'undefined';
                          singleProblem.description = 'This is a graph problem with regression task';
                          problemSets.push(singleProblem);
                        }
                        ...
                        for(var l = 0; l < metricMappings.clustering.length; l++){
                          ...
                          singleProblem.creationType = 'auto';
                          singleProblem.meaningful = 'undefined';
                          singleProblem.description = 'This is a graph problem with clustering task';
                          problemSets.push(singleProblem);
                        }
                        ...
                        for(var l = 0; l < metricMappings.linkPrediction.length; l++){
                          ...
                          singleProblem.creationType = 'auto';
                          singleProblem.meaningful = 'undefined';
                          singleProblem.description = 'This is a graph problem with link-prediction task';
                          problemSets.push(singleProblem);
                        }
                        ...
                        for(var l = 0; l < metricMappings.vertexNomination.length; l++){
                          ...
                          singleProblem.creationType = 'auto';
                          singleProblem.meaningful = 'undefined';
                          singleProblem.description = 'This is a graph problem with vertex-nomination task';
                          problemSets.push(singleProblem);
                        }

                      }//end of if for communityDetection
                      else if(graphTaskMappings.integer[k] == 'communityDetection'){
                        for(var l = 0; l < metricMappings.communityDetection.length; l++){
                          singleProblem = {};
                          count++;
                          singleProblem.problemID = count;
                          ...
                          singleProblem.creationType = 'auto';
                          singleProblem.meaningful = 'undefined';
                          singleProblem.description = 'This is a graph problem with community-detection task';
                          problemSets.push(singleProblem);
                        }
                        ...
                        for(var l = 0; l < metricMappings.graphClustering.length; l++){
                          ...
                          singleProblem.creationType = 'auto';
                          singleProblem.meaningful = 'undefined';
                          singleProblem.description = 'This is a graph problem with grtaph-clustering task';
                          problemSets.push(singleProblem);
                        }
                        ...
                        for(var l = 0; l < metricMappings.collaborativeFiltering.length; l++){
                          ...
                          singleProblem.creationType = 'auto';
                          singleProblem.meaningful = 'undefined';
                          singleProblem.description = 'This is a graph problem with collaborative-filtering task';
                          problemSets.push(singleProblem);
                        }
      ...
      if(totalGraphs > 1){
        ...
        for(var i = 0; i < metricMappings.graphMatching.length; i++){
          ...
          singleProblem.creationType = 'auto';
          singleProblem.meaningful = 'undefined';
          singleProblem.description = 'This is a graph problem with more than one graphs and with graph-matching task';
          problemSets.push(singleProblem);
  ...
  else if ( (_.includes(dataDescMetadata.resourceType.type, 'table')) || (_.includes(dataDescMetadata.resourceType.type, 'text')) || (_.includes(dataDescMetadata.resourceType.type, 'image')) || (_.includes(dataDescMetadata.resourceType.type, 'audio')) || (_.includes(dataDescMetadata.resourceType.type, 'video')) || (_.includes(dataDescMetadata.resourceType.type, 'speech')) || (_.includes(dataDescMetadata.resourceType.type, 'timeseries')) ){
    ...
    for(var i = 0; i < dataDescMetadata.columnType.id.length; i++){
      ...
      else if(_.includes(dataDescMetadata.resourceType.type, 'audio') && dataDescMetadata.columnType.resObject[i] == 'item'){
        resObject = 'audio';
      }
      else if(_.includes(dataDescMetadata.resourceType.type, 'video') && dataDescMetadata.columnType.resObject[i] == 'item'){
        resObject = 'video';
      }
      else if(_.includes(dataDescMetadata.resourceType.type, 'speech') && dataDescMetadata.columnType.resObject[i] == 'item'){
        resObject = 'speech';
      }
      else if(_.includes(dataDescMetadata.resourceType.type, 'image') && dataDescMetadata.columnType.resObject[i] == 'item'){
        resObject = 'image';
      }
      else if(_.includes(dataDescMetadata.resourceType.type, 'timeseries') && dataDescMetadata.columnType.resObject[i] == 'item'){
...
/////////
      ...
      if(dataDescMetadata.columnType.type[i] == 'categorical' || dataDescMetadata.columnType.type[i] == 'integer'){//} || dataDescMetadata.columnType.type[i] == 'real'){
        ...
        for (var k = 0; k < tablularTaskMappings.categorical.length; k++){
          if(tablularTaskMappings.integer[k] == 'classification'){
              for(var l = 0; l < metricMappings.classification.length; l++){
                ...
                singleProblem.creationType = 'auto';
                singleProblem.meaningful = 'undefined';
                singleProblem.description = 'This is a classification task with ' + resObject + ' field as prediction field';
                problemSets.push(singleProblem);
              }
            ...
            for(var l = 0; l < metricMappings.clustering.length; l++){
              ...
              singleProblem.description = 'This is a clustering task with ' + resObject + ' field as prediction field';
              problemSets.push(singleProblem);
            }

          }
          else if(tablularTaskMappings.integer[k] == "collaborativeFiltering"){
            for(var l = 0; l < metricMappings.collaborativeFiltering.length; l++){
              singleProblem = {};
              count++;
              singleProblem.problemID = count;
          ...
          if(_.includes(dataDescMetadata.columnType.type, 'dateTime') && dataDescMetadata.columnType.type[i] != 'dateTime'){
            for(var l = 0; l < metricMappings.timeSeriesForecasting.length; l++){
              ...
              singleProblem.creationType = 'auto';
              singleProblem.meaningful = 'undefined';
              singleProblem.description = 'This is a time-series forcasting task where the time-series is one of the target field(s)';
              problemSets.push(singleProblem);
            }
          }
        ...
        if((_.includes(dataDescMetadata.resourceType.type, 'image') || _.includes(dataDescMetadata.resourceType.type, 'audio') || _.includes(dataDescMetadata.resourceType.type, 'video') || _.includes(dataDescMetadata.resourceType.type, 'speech')) && dataDescMetadata.columnType.resObject[i] != 'item'){
          for(var l = 0; l < metricMappings.objectDetection.length; l++){
            ...
            singleProblem.creationType = 'auto';
            singleProblem.meaningful = 'undefined';
            singleProblem.description = 'This is an object Detection task with image/audio/video/speech field as one of the target field(s)';
            problemSets.push(singleProblem);
          }
        }

        if(_.includes(dataDescMetadata.resourceType.type, 'timeseries') && dataDescMetadata.columnType.resObject[i] != 'item'){
          for(var l = 0; l < metricMappings.timeSeriesForecasting.length; l++){
            singleProblem = {};
            count++;
            singleProblem.problemID = count;
      ...
      else if(dataDescMetadata.columnType.type[i] == 'string' || dataDescMetadata.columnType.type[i] == 'boolean'){
        ...
        for (var k = 0; k < tablularTaskMappings.categorical.length; k++){
          ...
          else if(tablularTaskMappings.string[k] == "collaborativeFiltering"){
            for(var l = 0; l < metricMappings.collaborativeFiltering.length; l++){
              singleProblem = {};
              count++;
              singleProblem.problemID = count;
        ...
        if((_.includes(dataDescMetadata.resourceType.type, 'image') || _.includes(dataDescMetadata.resourceType.type, 'audio') || _.includes(dataDescMetadata.resourceType.type, 'video') || _.includes(dataDescMetadata.resourceType.type, 'speech')) && dataDescMetadata.columnType.resObject[i] != 'item'){
          for(var l = 0; l < metricMappings.objectDetection.length; l++){
            ...
            singleProblem.creationType = 'auto';
            singleProblem.meaningful = 'undefined';
            singleProblem.description = 'This is an object Detection task with image/audio/video/speech field as one ofn the target field(s)';
            problemSets.push(singleProblem);
          }
        ...
        if(_.includes(dataDescMetadata.resourceType.type, 'timeseries') && dataDescMetadata.columnType.resObject[i] != 'item'){
          for(var l = 0; l < metricMappings.timeSeriesForecasting.length; l++){
           ...
           if(tablularTaskMappings.dateTime[k] == 'classification'){
            ...
            else if(tablularTaskMappings.dateTime[k] == "collaborativeFiltering"){
              for(var l = 0; l < metricMappings.collaborativeFiltering.length; l++){
                singleProblem = {}
                count++;
                singleProblem.problemID = count;
...
var probleSetsGeneration = function(finalProblemSets, dataDescMetadata){
    ...
    for (var i = 0; i < finalProblemSets.length; i++){
      ...
      var problemSetFile = {
        about: {
            problemID: dataDescFile.about.datasetName + '_Problem_' + finalProblemSets[i].problemID,
      ...
      for(var j = 0; j < dataDescMetadata.columnType.id.length; j++){
        if(finalProblemSets[i].targetFeature == dataDescMetadata.columnType.name[j]){
          predictionsArray.colIndex = Number(dataDescMetadata.columnType.id[j]);

          for(var k = 0; k < dataDescMetadata.resourceType.id.length; k++){
            if(dataDescMetadata.columnType.resObject[j] == 'null' && dataDescMetadata.resourceType.type[k] == 'table'){
                predictionsArray.resID = dataDescMetadata.resourceType.id[k].toString();
            }
            else if(dataDescMetadata.columnType.resObject[j] != 'null' && dataDescMetadata.resourceType.type[k] != 'table'){
                predictionsArray.resID = dataDescMetadata.resourceType.id[k].toString();
            }
      ...
      if(i == 0){
        if (!fs.existsSync(user_problems_root)){
          ...
          console.log("New Directory -- " + user_problems_root + " -- has been created for problem set");
        }
        else {
          console.log("re-creating folder at", user_problems_root);
          fse.removeSync(user_problems_root);
          fs.mkdirSync(user_problems_root);
          console.log("Directory -- " + user_problems_root + " -- has been re-created for problem set");
        }
      }
      ...
      jsonfile.writeFileSync(filePath, problemSetFile, function (err) {
          if(err){
            ...
            console.log("Problem Description JSON File has been created at "+problemDir+" directory");
          }
      })
      ...
      if(finalProblemSets[i].meaningful == 'yes' || finalProblemSets[i].meaningful == 'no'){
      ...
      serachSolutionRequest.problemSetSerachSolutionRequest(problemSetFile, problemDir);
    ...
    csvWriter.writeRecords(problemLabels)       // returns a promise
    .then(() => {
        console.log('labels.csv file has been created!');
    });

    console.log("Probel Set JSON files and the corresponding SearchSolutionsRequest JSON files have been created sucessfully.!!!");
