...
module.exports.set = function(session, socket) {
  ...
  function getGoldData() {
    return new Promise(function(resolve, reject) {
      if (reading) {
        ...
        // console.log("READ");
        reading = true;
        let groundTruthFile = session.getCurrentDataset().getDatasetPath();
        ...
        readCsvValues(valueColName, groundTruthFile).then(valueMap => {
          console.log(
            "collaborative filtering: gold answers loaded, # of rows: ",
            valueMap.size
          );
  ...
  function doResiduals(data) {
    ...
    readCsvValues(valueColName, predictionsFile).then(results => {
      console.log(
        "collaborative filtering: predictions file",
        predictionsFile,
        "loaded, # of rows: ",
        results.size
      );
