...
function handleTimeseriesData(session, socket) {
  ...
  symlinkDir(
    handleUrl(datasetPath + "/tables/learningData.csv"),
...
module.exports.set = function(session, socket) {
  session.registerDatasetUpdates(dataset => {
    if (dataset) {
      handleTimeseriesData(session, socket);
    }
  });
