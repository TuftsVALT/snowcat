...
const problemSetSerachSolutionRequest = require(appRoot +
  "/relay/functions/problemSetSerachSolutionRequest.js");
...
var preprocessProblemDesc = function(dataset) {
  ...
  */
  ...
  var dataDescFile = dataset.getDatasetSchema();
...
var probleSetsGeneration = function(
  finalProblemSets,
  dataDescMetadata,
  session
) {
  ...
  console.log(
    "HELP, I AM CLUELESS; ASSUMING '/output/problem_desc' AS THE PROBLEM_ROOT FOLDER!!"
  );
  // }
  ...
  if (!fs.existsSync("/output/")) {
    ...
    if (process.env["D3MOUTPUTDIR"]) {
      user_problems_root = process.env["D3MOUTPUTDIR"] + "/problems";
    } else {
      user_problems_root = appRoot + "/output/problems";
    }
  }
  ...
  for (var i = 0; i < finalProblemSets.length; i++) {
    ...
    var problemSetFile = {
      about: {
        ...
        //problemURI is optional
        //problemURI: ' ',
        taskType: finalProblemSets[i].taskType, // make it an array Alex
    ...
    //This is to assign column names as keywords for augmented data
    for (var j = 0; j < dataDescMetadata.columnType.id.length; j++) {
      problemSetFile.dataAugmentation.keywords.push(
        dataDescMetadata.columnType.name[j]
      );
    }

    problemSetFile.dataAugmentation.domain.push("unknown");
    ...
    var filePath = problemDir + "/problem_schema.json";
    ...
    //serachSolutionRequest.problemSetSerachSolutionRequest(currentSession, problemSetFile, problemDir);
    // serachSolutionRequest.problemSetSerachSolutionRequest(herald, problemSetFile, problemDir);
    problemSetSerachSolutionRequest(session, problemSetFile, problemDir);

    //console.log("WARNING: SEARCH SOLUTION REQUEST COMMENTED OUT!!");
...
module.exports.set = function(session, socket) {
  session.registerDatasetUpdates(dataset => {
    if (dataset) {
      ...
      socket.emit("problemSetProcessed", []);
    }
  });
  ...
  */
  ...
  socket.on("problemDiscoveryJSONCreation", function(data) {
    // @todo we have to change this later on to make valid api output - alex
    let returnSet = preprocessProblemDesc(session.getCurrentDataset());
    // console.log("session.getCurrentHerald():"+session.getCurrentHerald());
    // probleSetsGeneration(data, returnSet.dataDescMetadata, session); //, session.getCurrentHerald());
  });
};
