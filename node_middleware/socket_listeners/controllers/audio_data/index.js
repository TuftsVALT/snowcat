...
function requestAudios(dataset, socket) {
  // const staticPath = evaluationConfig.training_data_root + "/";
  const staticPath = dataset.getDatasetPath() + "/";
  let datasetSchema = dataset.getDatasetSchema();
  // let rootPath = dataset.getDatasetPath() + "/";
...
module.exports.set = function(session, socket) {
  session.registerDatasetUpdates(function(dataset) {
    requestAudios(dataset, socket);
  });
  socket.on("requestAudios", () => {
    console.log("requestAudios is on !!!");
